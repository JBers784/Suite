...
namespace Suite.Models.Almacenes
{
    public class MBasxCC
    {
        ...
        public MBasxCC() { }
        public List<MBasxCC> Listar(string ctaccosto, int mes, int año)
        {
            AlmacenesEntities entities = new AlmacenesEntities();
            List<MBasxCC> resp = new List<MBasxCC>();
            try
            {
                if (!string.IsNullOrEmpty(ctaccosto))
                {
                    resp = (from munprod in entities.Mundo_Productos
                            ...
                            where ((salprod.CtaCCosto).Trim() == ctaccosto) && (salprod.SalAlm_Fecha.Year == año) && (munprod.mProducto_Status == "B") //&& (salprod.SalAlm_Fecha.Month == mes)
