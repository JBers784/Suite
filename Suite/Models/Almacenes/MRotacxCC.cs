...
namespace Suite.Models.Almacenes
{
    public class MRotacxCC
    {
        ...
        public MRotacxCC() { }
        public List<MRotacxCC> Listar(string ctaccosto, int mes, int año)
        {
            AlmacenesEntities entities = new AlmacenesEntities();
            List<MRotacxCC> resp = new List<MRotacxCC>();
            try
            {
                if (!string.IsNullOrEmpty(ctaccosto))
                {
                    resp = (from munprod in entities.Mundo_Productos
                            ...
                            where ((salprod.CtaCCosto).Trim() == ctaccosto) && (salprod.SalAlm_Fecha.Year == año) && (munprod.mProducto_Status == "R") //&& (salprod.SalAlm_Fecha.Month == mes)
                            orderby almec.Almacen_Descrip, munprod.mProducto_Descrip, salprod.SalAlm_id ascending
                            select new MRotacxCC
                            {
